    ...
    setup(_, { emit }) {
      ...
      const getYarnQueueFormSchema = computed((): FormSchema[] => {
        return [
          {
            field: 'queueLabel',
            label: t('flink.setting.yarnQueue.yarnQueueLabelExpression'),
            component: 'Input',
            componentProps: {
              disabled: isUpdate.value,
              placeholder: t('flink.setting.yarnQueue.placeholder.yarnQueueLabelExpression'),
            },
          ...
          },
          {
            ...
            rules: [{ max: 512, message: t('flink.setting.yarnQueue.descriptionMessage') }],
          },
        ];
      });
