...
public class AppBuildPipeServiceImpl
    ...
    public boolean buildApplication(@Nonnull Application app) {
                        ...
                        if (app.isCustomCodeJob()) {
                            ...
                            if (app.isUploadJob()) {
                                ...
                                checkOrElseUploadJar(
                                        app.getFsOperator(), localJar, uploadJar, appUploads);
                            ...
                            if (!app.getDependencyObject().getJar().isEmpty()) {
                                ...
                                for (String jar : app.getDependencyObject().getJar()) {
                                    ...
                                    checkOrElseUploadJar(
                                            FsOperator.lfs(), localJar, uploadJar, localUploads);
                                }
                            }
                        }
                    }
        ...
        if (PipelineType.FLINK_NATIVE_K8S_APPLICATION == pipeline.pipeType()) {
            pipeline.as(FlinkK8sApplicationBuildPipeline.class)
                    .registerDockerProgressWatcher(
                            new DockerProgressWatcher() {
                                @Override
                                public void onDockerPullProgressChange(
                                        DockerPullSnapshot snapshot) {
                                    DOCKER_PULL_PG_SNAPSHOTS.put(app.getId(), snapshot);
                                }

                                @Override
                                public void onDockerBuildProgressChange(
                                        DockerBuildSnapshot snapshot) {
                                    DOCKER_BUILD_PG_SNAPSHOTS.put(app.getId(), snapshot);
                                }

                                @Override
                                public void onDockerPushProgressChange(
                                        DockerPushSnapshot snapshot) {
                ...
                switch (app.getApplicationType()) {
                    case STREAMPARK_FLINK:
                        return String.format(
                                "%s/%s", app.getAppLib(), app.getModule().concat(".jar"));
                    case APACHE_FLINK:
    ...
    private void checkOrElseUploadJar(
        ...
        if (!fsOperator.exists(targetJar)) {
            ...
            // The file exists to check whether it is consistent, and if it is inconsistent,
            // re-upload it
