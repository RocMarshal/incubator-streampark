...
public class Throws {
  ...
  private static final ConcurrentMap<Tuple2<Class<?>, Class<?>>, Constructor<?>> CACHE =
      new ConcurrentHashMap<Tuple2<Class<?>, Class<?>>, Constructor<?>>() {};
  ...
  private static <T extends RuntimeException> T createRuntimeExceptionInternal(
    ...
    Constructor<?> constructor = CACHE.get(key);
    try {
      if (constructor == null) {
        CACHE.put(
            key,
            hasElements(errorMsgs)
                ? exceptionClass.getConstructor(String.class)
                : exceptionClass.getConstructor());
      }
      constructor = CACHE.get(key);
    } catch (Exception e) {
      throw new RuntimeException(e);
    }
    AssertUtils.notNull(
